...
```{r}
apCancelOrig <- cancellations %>% group_by(Origin) %>% summarize(
  n = n()
)
apCancelOrig <- merge(apCancelOrig, 
                      unique(airports[, c("LOCID", "LONGITUDE", "LATITUDE")]),
                      by.x="Origin", by.y="LOCID")
ggplot(data=apCancelOrig) + geom_point(aes(LONGITUDE, LATITUDE, size=n))
table(apCancelOrig$Origin)

apCancelDest <- cancellations %>% group_by(Dest) %>% summarize(
  n = n()
)
apCancelDest <- merge(apCancelDest, airports[, c("LOCID", "LONGITUDE", "LATITUDE")],
                      by.x="Dest", by.y="LOCID")
ggplot(data=apCancelDest) + geom_point(aes(LONGITUDE, LATITUDE, size=n))
table(apCancelDest$Dest)
```
