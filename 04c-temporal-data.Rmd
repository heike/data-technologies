...
Temporal data uses time as a fundamentally contextual variable to index different positional states in past-to-future order.
...
as.Date(x)
as.Date(x, tz = "Australia/Melbourne")
as_date(x)
```

## Steps counting towards the end of PhD

Admittedly, it's a clickbait title: we're not going to discuss steps required for getting PhD done, instead explore more on personal health data--step counts, in Dr Wang's final year of her PhD. A richer depiction of individual agendas can be built up by simply slicing and dicing temporal variables. This data set is tidy and *nearly clean*, with two variables: `date_time` at hourly interval and the associated `count`.
...
## Calibrating time zones: a night owl?

Is Dr Wang a night owl, wandering around ghostly in late evenings? Yes, if I'd have been in GMT+0. But no, back then I was based in Melbourne, Australia. The data was exported in UTC, and read in UTC as default by `readr::read_csv()`.

```{r}
tz(step_count[["date_time"]])
```

Two functions in **lubridate** turn out useful in tackling time zones:

* `with_tz()`: change the clock time and adapt to new time zone.
* `force_tz()`: keep the clock time as is but overwritten with new time zone. Its sibling `force_tzs()` can pass multiple time zones at once.

We convert UTC to "Australia/Melbourne" using `with_tz()`. The timestamps show that my movement is rather normal.
...
Time can be aggregated to higher levels in the temporal hierarchy, for example sub-daily resolutions to daily/weekly/monthly/quarterly/annually. By aggregation, it makes easier to grasp signals that might have been buried in lower-level details otherwise. Does Dr Wang set a fitness goal, say 10000 steps per day? "Apparently not", the plot answers. The horizontal dashed line sets the daily step goal, which has been randomly hit by 108 bars representing counts.
...
library(tsibble)
library(patchwork)
monthly_step_count <- step_count %>% 
  group_by(yrmth = yearmonth(date_time)) %>% 
  summarise(monthly_count = sum(count)) %>% 
  mutate(avg_count = monthly_count / days_in_month(yrmth))

p_sum <- monthly_step_count %>% 
  ggplot(aes(x = yrmth, y = monthly_count)) +
  geom_col(colour = "white", fill = "#e6550d")

p_avg <- monthly_step_count %>% 
  ggplot(aes(x = yrmth, y = avg_count)) +
  geom_line() +
  geom_point() +
  ylim(c(0, 10000))
p_sum + p_avg
