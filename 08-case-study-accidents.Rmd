...
library(dplyr)
numbers <- data.frame(
  year = accidents %>% purrr::map(.f = function(x) {
    year <- x$YEAR[1]
    if (year < 2000) return(1900+year[1])
    year
  }) %>% unlist,
  fatalities = accidents %>% purrr::map_dbl(.f = function(x) {
    sum(x$FATALS)
  }),
  n = accidents %>% purrr::map(nrow) %>% unlist
)
write.csv(numbers, "data/numbers.csv", row.names=FALSE)
```

```{r fatalities-trend, fig.keep=TRUE, fig.cap="Number of accidents and fatalities over time.", out.width='80%', fig.asp=.75, fig.align='center', message=FALSE, warning = FALSE}
...
```{r}
qplot(year, number, data=vehicles) + ggtitle("Number of vehicles registered")  
numbers <- merge(numbers, vehicles, by="year", all.x=TRUE)
qplot(year, n/number, data=numbers, colour=mode) + ggtitle("Fatal accidents by number of registered vehicles")
```
...
+ Was there somethings wrong with the vehicle? Investigate variable ```MFACTOR``` in the ```Factor.dbf```. Make sure to re-code the numbers to strings. 
