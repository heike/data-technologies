...
#### The process
We want to scrape the daily information from Weather Underground for a particular station and some determined time frame. 
...
library(ggplot2)
qplot(data=temps, Previous, Actual, geom="jitter", alpha=I(0.5)) + 
  xlab("Yesterday's maximum temperature") +
  ylab("Today's maximum temperature") + 
  geom_smooth() + 
  coord_equal()

qplot(data=temps, Previous, Actual, geom="hex") + 
  xlab("Yesterday's maximum temperature") +
  ylab("Today's maximum temperature") +
  coord_equal() 
```
...
#  browser()
  if (is.null(day.stats$Actual)) return(NULL)
  day.stats$Unit <- str_extract(day.stats$Actual, pattern = units)
  day.stats$Actual <- as.numeric(str_extract(day.stats$Actual, pattern = numbers))
  day.stats$Average <- as.numeric(str_extract(day.stats$Average, pattern = numbers))
  day.stats$RecordYear <-
...
last <- as.Date("2000-08-25")

last <- days[dateIDX]
first <- as.Date("1998-01-01")
days <- seq(first, last, "day")
...
Rob is actually using conditional densities: given today's temperature what is the density distribution for tomorrow's temperatures?
We are then using highest-density region boxplots to show the results. The location of the dot indicates the location of the density's highest mode.
...
library(hdrcde)
temps.cde <- cde(subtemps$Previous[-nrow(subtemps)], subtemps$Actual[-nrow(subtemps)], nxmargin=30)
