...
And now it's time for the analysis:
```{r, warning=FALSE}
...
qplot(data=temps, Previous, Actual, geom="hex") + 
  ...
  coord_equal()
```
What we see is a strong linear pattern, i.e. tomorrow's maximum temperature is close to what we see today. There is a peak in density at temperatures around 30 degree Celsius. With higher temperatures we see less variability, while at lower temperatures this relationship is more variable. 
